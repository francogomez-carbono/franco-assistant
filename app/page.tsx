    ...
    <main className="min-h-screen bg-[#050505] text-white font-sans selection:bg-purple-500/30 pb-20">
      <div className="max-w-md mx-auto flex flex-col p-5">
        ...
        <header className="mb-8 pt-4 text-center">
          ...
          {/* BARRA DE NIVEL GLOBAL (ESTILO HERO) */}
        ...
        {/* STATS GRID (ESTILO CLÁSICO COMPACTO) */}
        ...
        {/* BITÁCORA (AL FINAL) */}
        <section>
          <h3 className="text-[#444] text-[10px] font-bold tracking-[0.2em] mb-4 uppercase text-center">Bitácora de Logros</h3>
          <div className="space-y-2">
            {logros.length === 0 ? (
              <div className="text-neutral-800 text-xs text-center py-8">Sin actividad reciente.</div>
            ) : (
              logros.map((log) => (
                <div key={log.id} className="bg-[#0a0a0a] border border-[#222] p-3 rounded-lg flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-[#111] border border-[#222] flex items-center justify-center text-[10px] text-green-500">✓</div>
                    <span className="text-neutral-400 text-xs font-medium truncate max-w-[180px]">{log.tarea}</span>
                  </div>
                  <span className="text-[10px] font-bold text-[#333] bg-[#111] px-2 py-1 rounded border border-[#222]">+{log.xpGanada} XP</span>
                </div>
              ))
            )}
          </div>
...
// El componente exacto de la imagen que te gustó
function StatCard({ emoji, title, lvl, xp, meta, progress, color }: any) {
  return (
    <div className="bg-[#111] border border-[#222] p-4 rounded-xl shadow-sm">
